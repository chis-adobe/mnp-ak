...
async function loadComponent(componentName, element, fd, container, formId) {
  ...
  if (status !== 'loading' && status !== 'loaded') {
    ...
    try {
      loadCSS(`${window.hlx.codeBasePath}/blocks/embedded-form/components/${componentName}/${componentName}.css`);
      const decorationComplete = new Promise((resolve) => {
        (async () => {
          try {
            const mod = await import(
              `${window.hlx.codeBasePath}/blocks/embedded-form/components/${componentName}/${componentName}.js`
            );
            if (mod.default) {
