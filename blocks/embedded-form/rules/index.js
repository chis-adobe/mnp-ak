...
async function initializeRuleEngineWorker(formDef, renderHTMLForm) {
  ...
  const myWorker = new Worker(`${window.hlx.codeBasePath}/blocks/embedded-form/rules/RuleEngineWorker.js`, { type: 'module' });
...
export async function initAdaptiveForm(formDef, createForm) {
  ...
  await registerCustomFunctions(formDef?.properties?.customFunctionsPath || '/blocks/embedded-form/functions.js', window.hlx?.codeBasePath);
