...
const EMAIL_ICON = '<svg class="office-info-icon" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20 4H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2zm0 4-8 5-8-5V6l8 5 8-5z"/></svg>';
...
async function buildInfoHeader(address) {
  ...
  if (address.email) {
    const li = document.createElement('li');
    li.innerHTML = `${EMAIL_ICON}<a href="mailto:${address.email}">Email: ${address.email}</a>`;
    contactList.append(li);
  }
...
export default async function init(el) {
  ...
  if (city) {
    address = {
      ...
      'toll-free': getMetadata('toll-free') || '',
      fax: getMetadata('fax') || '',
      email: getMetadata('email') || '',
    };
  } else {
